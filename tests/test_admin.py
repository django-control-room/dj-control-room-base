"""
Tests for Django Admin integration with DJ control room base.

The DJ control room base integrates with Django Admin through a placeholder model
...
class TestAdminIntegration(BasePanelTestCase):
    ...
    def test_celery_panel_appears_in_admin_index(self):
        ...
        self.assertContains(response, "dj_control_room_base")

        # Check that the link to the changelist exists
        changelist_url = reverse(
            "admin:dj_control_room_base_basepanelplaceholder_changelist"
        )
    ...
    def test_celery_panel_changelist_redirects_to_index(self):
        ...
        expected_url = reverse("dj_control_room_base:index")
