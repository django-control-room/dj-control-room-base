PACKAGE_NAME = dj_control_room_base
...
test_install: build
	python -m pip uninstall -y $(PACKAGE_NAME) || true
	python -m pip install -e .
	python -c "import dj_control_room_base; print('✅ Import success!')"
...
test_coverage:
	...
	@python -m pytest --cov=dj_control_room_base --cov-report=xml --cov-report=html --cov-report=term-missing tests/
...
test_coverage_docker:
	...
	@docker compose exec dev bash -c "cd /app && REDIS_HOST=redis python -m pytest --cov=dj_control_room_base --cov-report=xml --cov-report=html --cov-report=term-missing tests/"
...
docker_up:
	...
	@echo "   Dev container: dj-control-room-base-dev-1"
	@echo ""
